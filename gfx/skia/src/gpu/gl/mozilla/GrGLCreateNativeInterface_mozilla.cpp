...
GrGLInterface* CreateGrInterfaceFromGLContext(GLContext* context)
{
    ...
    const char* versionString = (const char*) context->fGetString(GR_GL_VERSION);
    const char* extString = (const char*) context->fGetString(GR_GL_EXTENSIONS);
...
// We don't support creating an interface without a GLContext
