...
gfxCairoFontBase::gfxCairoFontBase(cairo_scaled_font_t *aScaledFont,
                                   gfxFontEntry *aFontEntry,
                                   const gfxFontStyle *aFontStyle)
    : gfxFT2FontBase(NULL, aFontEntry, aFontStyle),
      mSpaceGlyph(0),
      mHasMetrics(false)
{
...
gfxCairoFontBase::~gfxCairoFontBase()
{
    cairo_scaled_font_destroy(mScaledFont);
}

PRUint32
gfxCairoFontBase::GetGlyph(PRUint32 aCharCode)
{
    ...
    if (slot->mCharCode != aCharCode) {
        ...
        slot->mGlyphIndex = gfxCairoLockedFace(this).GetGlyph(aCharCode);
    }
...
void
gfxCairoFontBase::GetGlyphExtents(PRUint32 aGlyph, cairo_text_extents_t* aExtents)
{
...
gfxCairoFontBase::GetMetrics()
{
    ...
    if (NS_UNLIKELY(GetStyle()->size <= 0.0)) {
        ...
        gfxCairoLockedFace(this).GetMetrics(&mMetrics, &mSpaceGlyph);
    }
...
PRUint32
gfxCairoFontBase::GetSpaceGlyph()
{
...
hb_blob_t *
gfxCairoFontBase::GetFontTable(PRUint32 aTag)
{
    hb_blob_t *blob;
    ...
    bool haveTable = gfxCairoLockedFace(this).GetFontTable(aTag, buffer);
...
PRUint32
gfxCairoFontBase::GetGlyph(PRUint32 unicode, PRUint32 variation_selector)
{
    if (variation_selector) {
        PRUint32 id =
            gfxCairoLockedFace(this).GetUVSGlyph(unicode, variation_selector);
        if (id)
            return id;
    }
...
PRInt32
gfxCairoFontBase::GetGlyphWidth(gfxContext *aCtx, PRUint16 aGID)
{
...
bool
gfxCairoFontBase::SetupCairoFont(gfxContext *aContext)
{
