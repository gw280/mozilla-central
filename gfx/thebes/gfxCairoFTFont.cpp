...
gfxCairoFTFont::gfxCairoFTFont(cairo_scaled_font_t *aScaledFont,
      ...
      mFontExtents(nsnull)
{
    ...
    mFontExtents = new gfxFT2CairoExtents(mScaledFont);
}

gfxCairoFTFont::~gfxCairoFTFont()
{
    cairo_scaled_font_destroy(mScaledFont);
}

PRUint32
gfxCairoFTFont::GetGlyph(PRUint32 aCharCode)
{
    ...
    if (slot->mCharCode != aCharCode) {
        ...
        slot->mGlyphIndex = mFontExtents->GetGlyph(aCharCode);
    }
...
void
gfxCairoFTFont::GetGlyphExtents(PRUint32 aGlyph, cairo_text_extents_t* aExtents)
{
    return mFontExtents->GetGlyphExtents(this->GetStyle(), aGlyph, aExtents);
}

const gfxFont::Metrics&
gfxCairoFTFont::GetMetrics()
{
    if (mHasMetrics)
        return mMetrics;
...
hb_blob_t *
gfxCairoFTFont::GetFontTable(PRUint32 aTag)
{
    hb_blob_t *blob;
...
PRUint32
gfxCairoFTFont::GetGlyph(PRUint32 unicode, PRUint32 variation_selector)
{
    if (variation_selector) {
        PRUint32 id = mFontExtents->GetGlyph(unicode, variation_selector);
        if (id)
            return id;
    }
...
PRInt32
gfxCairoFTFont::GetGlyphWidth(gfxContext *aCtx, PRUint16 aGID)
{
...
bool
gfxCairoFTFont::SetupCairoFont(gfxContext *aContext)
{
