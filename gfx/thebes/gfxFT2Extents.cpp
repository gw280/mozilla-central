...
//
// Instead, let's just create a temporary cairo_scaled_font_t
// for this FT_Face and grab the cairo_text_extents_t for that.
//
// For performance reasons we can optionally pass in a
// cairo_scaned_font_t if we have one.
void
                                  ...
                                  PRUint32* aSpaceGlyph)
{
    ...
    if (FT_IS_SCALABLE(face)) {
        ...
        // font formats".
        gfxFloat emUnit = face->units_per_EM;
    ...
    *aSpaceGlyph = aFont->GetGlyph(' ', 0);
    ...
    if (face->underline_position && face->underline_thickness) {
        aMetrics->underlineSize = face->underline_thickness * yScale;
        ...
        if (post && post->underlinePosition) {
            ...
            aMetrics->underlineOffset = face->underline_position * yScale
